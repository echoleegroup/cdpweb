...
var EvadRoute = (function (_super) {
    ...
    EvadRoute.create = function (router) {
		...
		router.post('/SearchEvtpgID', function (req, res) {
			...
			var data =[] ;
			var p1 = new Promise(function(resolve, reject){db.query("SELECT evtpgID,msm_tpc,convert(varchar, sdt, 111)sdt,convert(varchar, edt, 111)edt FROM dm_EvtpgMst where client = '"+client+"'  and funcCatge = '"+funcCatge+"' order by msm_tpc asc ",function(err,recordset){
						if(err) {
							console.log(err);
							reject(2);
						}
		...
		router.post('/addEvtadTag', function (req, res) {
			...
			var tagID ;
			newtag = newtag.trim();
			var p1 = new Promise(function(resolve, reject){db.query("SELECT evtadID FROM dm_EvtadTag where evtadID="+evtadID+" and tagLabel ='"+newtag+"' " ,function(err,recordset){
					if(err) {
						console.log(err);
						reject(2);
					}
