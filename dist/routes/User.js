    ...
    userinfoRoute.create = function (router) {
        ...
        router.get('/UserInfoEdit', function (req, res) {
			if( req.session.userid && req.session.userid != ''){
				...
				var aduglist = [] ;
				var adug = [] ;
				var p1 = new Promise(function(resolve, reject){db.query('SELECT u.ugrpId,u.ugrpClass,u.ugrpName,u.remark,u.regdate,convert(varchar, u.modifyDate, 120)modifyDate,u.signer,u.isStop,uc.ugrpClassName FROM sy_ugrp u left join sy_ugrpClass uc on uc.ugrpClassId = u.ugrpClass order by u.ugrpClass asc' ,function(err,recordset){
						if(err) {
							console.log(err);
							reject(2);
						}
				...
				var p3 = new Promise(function(resolve, reject){db.query('SELECT userId,password,userName,email,convert(varchar,lastVisit, 120)lastVisit,telephone,convert(varchar,modifyTime, 120)modifyTime,convert(varchar,createdDate, 120)createdDate,bookmark,modifyName,uID,convert(varchar,loginTime, 120)loginTime,isstop from sy_infouser' + where ,function(err,recordset){
						if(err) {
							console.log(err);
							reject(2);
						}
