    ...
    FeedDataRoute.create = function (router) {
		...
		router.get('/FeedDataSearch', function (req, res) {
			...
			if( req.session.userid && req.session.userid != ''){
				var p1 = new Promise(function(resolve, reject){db.query("SELECT codeValue,codeLabel FROM sy_CodeTable where codeGroup = 'funcCatgeForFeedData' order by codeValue desc ",function(err,recordset){
							if(err) {
								console.log(err);
								reject(2);
							}
		...
		router.post('/FeedDataList', function (req, res) {
			...
			var feeddata = [] ;	
			if( req.session.userid && req.session.userid != ''){
				var p1 = new Promise(function(resolve, reject){db.query(unionSql ,function(err,recordset){
						if(err) {
							console.log(err);
							reject(2);
						}
		...
		router.post('/addFeedDataTag', function (req, res) {
			...
			var tagID ;
			newtag = newtag.trim();
			var p1 = new Promise(function(resolve, reject){db.query("SELECT outerListID FROM cu_OuterListTag where outerListID="+ListID+" and tagLabel ='"+newtag+"' " ,function(err,recordset){
					if(err) {
						console.log(err);
						reject(2);
					}
