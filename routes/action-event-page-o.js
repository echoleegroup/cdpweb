...
module.exports = (app) => {
	...
	router.post('/act/add_act',[middleware.check(),middleware.checkEditPermission(permission.EVENT_PAGE_EDIT)], function (req, res) {
		...
		var evtpgID = '';
		var p1 = new Promise(function (resolve, reject) {
			db.query("INSERT INTO dm_EvtpgMst(client,funcCatge,msm_tpc,url,sdt,edt,trkSdt,trkEdt,isDel,evtpgDesc,crtTime,updTime,updUser)" + values, function (err, recordset) {
				if (err) {
					console.log(err);
					reject(2);
				}
		...
		Promise.all([p1]).then(function (results) {
			db.query("SELECT Top 1 evtpgID FROM dm_EvtpgMst order by evtpgID desc ", function (err, recordset) {
				...
				res.redirect('/act/edit?evtpgID=' + evtpgID);
			});
